...
function GoogleReview({ className, usrAvatarUrl, username, date, amountOfStarts, reviewMessage }: SocialItemProps) {
  ...
  return (
    <li
      className={twMerge(
        "min-w-[300px] flex flex-col bg-foreground-accent hover:bg-brand/10 rounded-xl p-4",
        "border border-brand/20 hover:border-brand/30  duration-200",
      )}>
      {/* HEADER */}
      <div className="flex flex-col gap-3 mb-3">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-3">
            <Image
              className="w-10 h-10 rounded-full border-2 border-brand/20"
              src={usrAvatarUrl}
              alt="user avatar"
              width={40}
              height={40}
          ...
          <div className="bg-brand/10 p-1 rounded">
      ...
      <p className="text-title text-sm leading-relaxed">
        {displayedText}
        {isTruncated && (
          <Link className="text-brand hover:text-brand/80 inline ml-1 font-medium" href={businessInfo.mapUrl}>
            more
          </Link>
        )}
      </p>
...
export function Reviews() {
  return (
    <section className="bg-foreground rounded-xl border border-brand/20 p-6">
